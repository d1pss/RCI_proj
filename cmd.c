...
int process_command(char *input, Node_info* My_node){
    ...
    read_items = sscanf(input, "%s", cmd);

    //check if the command is the message command to read the arguments in a different way because the chat message can contain spaces
    if(!strcmp(cmd, "m")){
        read_items += sscanf(input, "%*s %s %[^\n]", arguments[0], chat_message);
    }else{
        read_items = sscanf(input, "%s %s %s %s", cmd, arguments[0], arguments[1], arguments[2]);
    }
...
int cmd_join(char* net_as_char, char* id_as_char, Node_info* My_node){
    ...
    if(My_node->is_in_net){
        //error already in a net must leave first
    ...
    /***************
    //Old section of code to check if id exists whith contact (legacy code)
    ...
    ***************/
    ...
    //create TCP server to accept connections from neighbors
...
int cmd_add_edge(char* dest_id_as_char, Node_info* My_node){
    ...
    //check if the destination is already a neighbor
    ...
    //get the ip and port of the destination id from the UDP server
    return_code = get_id_info(dest_ip, dest_Port, &is_dest_id_in_net, net_as_char, dest_id_as_char, My_node);
    if(return_code != SUCCESS) return return_code;

    //check if the destination id is in the network
    ...
    //create TCP client and connect to the destination id
...
int cmd_leave(Node_info* My_node){
   ...
   //close all TCP connections with neighbors
    ...
    //if the node was added with direct join the node is not registered in the network no need to remove it from the server
...
int cmd_remove_edge(char* id_to_remove_as_char, Node_info* My_node){
    ...
    //close the TCP connection with the neighbor to inform him that the edge is being removed
    Close_TCP_Client(id_to_remove, My_node);

    //for every way to (i) node that depended on the removed edge lose the path to (i) and coordinate neighbors
...
int cmd_direct_add_edge(char* dest_id_as_char, char* idIP, char* idTCP, Node_info* My_node){
    ...
    //check if the node is in a network
    if(!My_node->is_in_net){
        printf("Interface Command -> Operation denied: Node must be in a network to perform this action. Run (j/dj) first or type 'help' for the full command list.\n");
        return SUCCESS;
    }
...
int cmd_exit(Node_info* My_node){
    ...
    //if the node is in a network leave it before exiting
    if(My_node->is_in_net){
